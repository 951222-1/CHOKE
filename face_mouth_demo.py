...
def create_face_landmarker(model_path=FACE_MODEL_PATH):
    ...
    if not os.path.exists(model_path):
        # 檢查是否有預設全局路徑
        alt_path = r"C:\Users\ph090\face_landmarker.task"
...
def draw_face_and_mouth_landmarks(frame, face_landmarks_list, draw_all_mesh=False):
    """
    在 OpenCV 畫面上標示嘴唇關鍵點與連線輪廓以及鼻尖/下巴點位
    """
    h, w, _ = frame.shape
    for face in face_landmarks_list:
        ...
        # 標示嘴部關鍵地標 (13 上唇, 14 下唇, 78 左嘴角, 308 右嘴角, 4 鼻尖, 152 下巴)
        key_indices = [
            (4, (0, 255, 255)),    # 鼻尖 (黃)
            (152, (0, 255, 255)),  # 下巴 (黃)
            (13, (0, 0, 255)),     # 上唇 (紅)
...
def main():
    ...
    while True:
        ...
        if result.face_landmarks:
            ...
            # 3. 畫嘴部與臉部關鍵地標
            draw_face_and_mouth_landmarks(frame, result.face_landmarks)
